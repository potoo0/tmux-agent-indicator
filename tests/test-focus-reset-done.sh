...
pane_option_after="$(get_pane_option "$PANE" "@agent_state")"
window_option_after="$(get_window_option "$WIN" "@agent_state")"
...
assert_empty "$pane_option_after" "pane @agent_state should reset after done focus-in"
assert_empty "$window_option_after" "window @agent_state should reset after done focus-in"

pass "done focus reset clears styles, pane state, and options"
